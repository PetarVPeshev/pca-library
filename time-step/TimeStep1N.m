classdef TimeStep1N < handle
    ...
    methods
        function obj = TimeStep1N(t, params)
            ...
            arguments
                ...
                params.Vb       (:,1)   double = zeros(2, 1)
                params.w        (1,:)   double = double.empty(1, 0)
                params.h        (:,:,:) double = double.empty(2, 2, 0)
                params.tau_c    (1,1)   double = 0
                params.tau_s    (1,1)   double = 0
                params.tau_d    (1,:)   double = zeros(1, 2)
    ...
    methods(Access = protected)
        function setup(obj)
            ...
            obj.W0     = obj.w(1) * eye(obj.numFeeds);
            obj.H0     = obj.h(:, :, 1);
        ...
        function validate_properties(obj)
            ...
            mustBeValidTimeStepMatrix(obj.Vb,    [obj.numFeeds 1]);
            mustBeValidTimeStepMatrix(obj.w,     [1 obj.m_max]);
            mustBeValidTimeStepMatrix(obj.h,     [obj.numFeeds obj.numFeeds obj.m_max]);
            mustBeValidTimeStepMatrix(obj.tau_d, [1 obj.numFeeds]);
        end
    end
...
%             obj.reset();
    ...
    methods (Static)
        function Fs = compute_Fs(t, K, sigma_t, tau_c, tau_d)
            ...
            Fs = NaN(numFeeds, m_max);
            parfor feedIdx = 1 : numFeeds
                hm_n = exp(- 0.5 * ((tN - tau_d(feedIdx)) / sigma_t) .^ 2) .* exp(- (tM - tN) / tau_c);
                Fs(feedIdx, :) = sum(tril(hm_n), 2)';
            end
