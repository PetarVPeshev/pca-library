...
figure('Position', [680 558 800 420]);

for Nf_idx = 1 : 1 : length(Nf)
    plot(t * 1e12, gin(Nf_idx, :), LineSpec(Nf_idx), 'LineWidth', 1.5, 'Color', Color{Nf_idx}, ...
        'MarkerIndices', 1 : 100 : length(t), 'DisplayName', [DisplayName{Nf_idx} ' f_{i} = ' ...
        num2str(fi(Nf_idx) * 1e-12) ' THz, N_{f} = ' num2str(Nf(Nf_idx))]);
    hold on;
end
hold off;
